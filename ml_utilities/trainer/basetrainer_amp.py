...
import pandas as pd
import torch
from ml_utils.config import ResumeTrainingConfig, TrainingStrategyConfig
...
class BaseTrainer(Runner, ABC):
    ...
    def __init__(
        ...
        assert self._lr_scheduler_step in [
            'step', 'epoch'
        ], 'lr_scheduler_step must be either "step" or "epoch".'
        ...
        enable_gradscaler = self._training_strategy.enable_mixed_precision \
        ...
        LOGGER.info(
            f'Logging experiment data to directory: {self._experiment_dir}.')
    ...
    def _reset_metrics(self, which: str = 'all') -> None:
        if self._train_metrics is not None and (which == 'all'
                                                or which == 'train'):
            self._train_metrics.reset()
        if self._val_metrics is not None and (which == 'all'
                                              or which == 'val'):
    ...
    def _train_epoch(self, epoch: int) -> None:
        ...
        pbar = tqdm(self._loaders['train'],
                    desc=f'Train epoch {epoch}',
                    file=sys.stdout)
        for batch_idx, batch in enumerate(pbar):
            self._model.train()
            with Stopwatch() as sw:
                loss_dict = self._train_step(train_batch=batch,
                                             batch_idx=batch_idx)
            if self._train_step_idx % 10 == 0:  # do not log T_train_step every step #TODO make this configurable
                ...
                if n_tokens is not None:
                    time_dict[
                        'tokens_per_second'] = n_tokens / sw.elapsed_seconds
            ...
            if self._progress_measure == RUN_PROGRESS_MEASURE_STEP:
                if self._validation_and_early_stopping(
                        idx=self._train_step_idx,
    ...
    def _val_epoch(self, progress_idx: int, trained_model: nn.Module) -> float:
        ...
        pbar = tqdm(val_loader,
                    desc=f'Val after {self._progress_measure} {progress_idx}',
                    file=sys.stdout)
        with torch.no_grad():
            for xs, ys in pbar:
        ...
        self._logger.log_keys_vals(prefix='val',
                                   ...
                                   keys_val=metrics_epoch,
                                   log_to_console=True)
        val_score = metrics_epoch[next(iter(
            self._val_metrics.items()))[0]].item()
        self._reset_metrics('val')
        self._hook_on_val_epoch_end(progress_idx=progress_idx,
    ...
    def _create_checkpoint(self) -> None:
        ...
        self._logger.save_checkpoint(checkpoint,
                                     idx=idx,
                                     specifier=self._progress_measure)
    ...
    def _resume_from_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        ...
        # grad scaler
        if self._gradscaler is not None:
            self._gradscaler.load_state_dict(
                checkpoint['gradscaler_state_dict'])
        # scheduler
        if self._lr_scheduler is not None:
    ...
    def train(self) -> Dict[str, Any]:
        ...
        if self._resume_training:
            LOGGER.info(
                f'Resume from checkpoint: {str(self._resume_training)}')
            checkpoint = JobDirectory.load_resume_checkpoint(
                **asdict(self._resume_training), device=self.device)
        ...
        LOGGER.info(
            f'Starting training with progress measure `{self._progress_measure}`.'
        )
        ...
        self._best_val_score = float(
            'inf') if self._val_lower_is_better() else -float('inf')

        # validate untrained model as baseline
        self._model.eval()
        with Stopwatch() as sw:  # TODO make this a decorator
            self._best_val_score = self._val_epoch(self._epoch_idx,
                                                   self._model)
        ...
        final_results = {
            f'{PREFIX_BEST_CHECKPOINT}{self._progress_measure}':
            self._best_idx,
        ...
        if self._best_idx >= 0:
            ...
            self._logger.save_best_checkpoint_idx(
                specifier=self._progress_measure, best_idx=self._best_idx)
    ...
    def _validation_and_early_stopping(self, idx: int, specifier: str) -> bool:
        ...
        """
        model_saved = False
        if (self._save_every > 0 and idx % self._save_every
                == 0) or idx in self._save_every_idxes:
            self._create_checkpoint()
            model_saved = True
        ...
        if self._val_every > 0 and idx % self._val_every == 0:
            ...
            self._model.eval()
            with Stopwatch() as sw:
                val_score = self._val_epoch(progress_idx=idx,
                                            trained_model=self._model)
            ...
            if isnan(val_score):
                raise RuntimeError(
                    f'Validation score is NaN in {specifier} {idx}.')
